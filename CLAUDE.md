...
Claude Code and PI both read this file natively, so all agent instructions live
here - there is no separate `AGENTS.md`.
...
Reusable agent skills live in `.claude/skills/`. Claude Code reads them
natively; PI references the same directory via `.pi/settings.json`, so skills
are maintained in exactly one place. Add a skill as
`.claude/skills/<name>/SKILL.md`.

## Shared memory (OpenWolf)

This project uses [OpenWolf](https://github.com/cytostack/openwolf) for
cross-session memory and token tracking. Run `openwolf init` once per clone to
create the local `.wolf/` working dir and the Claude Code hooks.

OpenWolf's automatic tracking is Claude Code-only, but the memory files are plain
Markdown/JSON - **every agent should consult them**:

- `.wolf/cerebrum.md` - learned conventions, preferences, and a Do-Not-Repeat
  list. Read before generating code.
- `.wolf/anatomy.md` - project file map with token estimates. Check before
  reading a file in full.
- `.wolf/buglog.json` - known bugs and their fixes. Check before fixing an error.

The shared memory (`cerebrum.md`, `anatomy.md`, `buglog.json`, `OPENWOLF.md`,
`identity.md`) is committed; session logs, the token ledger, config, and hooks
are gitignored.

<!-- OpenWolf runtime memory. Activates once `openwolf init` has created .wolf/ in this clone. -->
@.wolf/OPENWOLF.md
